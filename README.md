...
Hesaplama kuralları [KURALLAR.md](KURALLAR.md) belgesinde tanımlıdır; motor bu kurallara **43 otomatik testle** bağlanmıştır. Canlı: **https://feraiz.com**
...
- Adım adım hesap açıklaması (kural maddesi referanslarıyla)
...
npm run kurallar    # KURALLAR.md değiştiyse public/kurallar.html'i yeniden üret
```

## Proje yapısı
- `src/lib/feraiz.js` — hesap motoru (saf fonksiyon; `KURALLAR.md`'ye bağlı, dil sözlüğü parametreli)
- `src/lib/feraiz.test.js` — kural maddelerine göre senaryo testleri
...
- `KURALLAR.md` — hesaplama kurallarının tek ve bağlayıcı kaynağı (Türkçe asıl metin)
- `public/` — statik dosyalar (kurallar.html, meal alt sitesi, privacy/terms, ads.txt)

## Katkı
Katkılar memnuniyetle karşılanır. Kurallar:
1. **Hesap davranışını değiştiren her PR, `KURALLAR.md` güncellemesi ve test içermelidir.** Kurallar belgesi ile motor asla ayrışamaz.
...
3. `npm run preview` ile smoke: hesap doğru, paylaş linki çalışıyor, 5 dil + RTL görünümü, print, `/kurallar.html`, `/meal/`, `/privacy.html`, `/ads.txt` erişilebilir
...
[KURALLAR.md](KURALLAR.md) (Turkish original is authoritative) and the engine is
...
## Contributing
1. **Any PR that changes calculation behaviour must update `KURALLAR.md` and the tests.** The engine and the rules document must never diverge.
